...
namespace App\Providers\SocialiteProviders\VKontakte;
...
class Provider extends AbstractProvider implements ProviderInterface
{
    ...
    protected function getUserByToken($token)
    {
        $response = $this->getHttpClient()->get(
            'https://api.vk.com/method/users.get?user_ids='.$token['user_id'].'&fields=uid,first_name,last_name,screen_name,photo_max_orig'
        );
    ...
    protected function mapUserToObject(array $user)
    {
        return (new User())->setRaw($user)->map([
            ...
            'email' => array_get($user, 'email'), 'avatar' => $user['photo_max_orig'],
        ]);
    }
